...
AcbotReplyMarkup = typing.Union[typing.List[typing.List[dict]], typing.List[dict], dict]
...
class Module:
    ...
    def __getattr__(self, name: str):
        if name in {"acbot_commands", "commands"}:
            return get_commands(self)

        if name in {"acbot_inline_handlers", "inline_handlers"}:
            return get_inline_handlers(self)

        if name in {"acbot_callback_handlers", "callback_handlers"}:
            return get_callback_handlers(self)

        if name in {"acbot_watchers", "watchers"}:
