...
async def answer(
    ...
    **kwargs,
    ...
    :param message: Message to answer to. Can be a tl message or acbot inline object
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if len(text) >= 4096 and not hasattr(message, "acbot_grepped"):
            try:
...
async def asset_channel(
    ...
    *,
    ...
    if _folder:
        ...
        try:
            folder = next(folder for folder in folders if folder.title == "acbot")
        except Exception:
            folder = None
...
def get_named_platform() -> str:
    ...
    return f"AuthorHost✍️ {os.environ['AUTHORHOST']}" if "AUTHORHOST" in os.environ else "💻 VDS"
...
def get_commit_url() -> str:
    """
    Get current acbot git commit url
    :return: Git commit url
    """
    try:
        hash_ = get_git_hash()
        return (
            f'<a href="https://github.com/VadymYem/AuthorBot/commit/{hash_}">#{hash_[:7]}</a>'
        )
    except Exception:
        return "Unknown"
