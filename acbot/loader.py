...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _acbot_client_id_logging_tag = copy.copy(
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?acbot$", from_id=659800858)
...
class Modules:
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.acbot_commands)
                inline_handlers.update(module.acbot_inline_handlers)
                callback_handlers.update(module.acbot_callback_handlers)
                watchers.extend(module.acbot_watchers.values())
    ...
    async def _register_modules(
        ...
        with contextlib.suppress(AttributeError):
            _acbot_client_id_logging_tag = copy.copy(self.client.tg_id)
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.acbot_commands))
            )

        for _command, cmd in instance.acbot_commands.items():
        ...
        for alias, cmd in self.aliases.copy().items():
            if cmd in instance.acbot_commands:
                self.add_alias(alias, cmd)

        self.register_inline_stuff(instance)

    def register_inline_stuff(self, instance: Module):
        for name, func in instance.acbot_inline_handlers.copy().items():
        ...
        for name, func in instance.acbot_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.acbot_watchers.values():
    ...
    async def _decline(
        ...
        self._db.set(
            "acbot.main",
            "declined_joins",
            list(set(self._db.get("acbot.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
    ...
    async def _request_join(
        ...
        if channel.id in self._db.get("acbot.main", "declined_joins", []):
            if assure_joined:
                raise LoadError(
        ...
        _module.strings._base_strings["_acbot_internal_request_join"] = (
        ...
        _module.acbot_wait_channel_approve = (
        ...
        with contextlib.suppress(AttributeError):
            delattr(_module, "acbot_wait_channel_approve")
    ...
    def get_prefix(self) -> str:
        return self._db.get("acbot.main", "command_prefix", ".")
    ...
    async def _mod_import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?acbot_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?acbot_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires acbot version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"acbot.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("acbot.main", "stats", True)
            and url is not None
