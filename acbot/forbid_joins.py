...
# list and you will be banned from AuthorChe's federation.
        ...
        for item in request:
            if item.CONSTRUCTOR_ID in {615851205, 1817183516} and next(
                (
                    ...
                    # APIRatelimiterMod is a core proxy, so it wraps around every module in acbot, if installed
                    # ForbidJoinMod is also a Core proxy, so it wraps around every module in acbot, if installed
                    # LoaderMod prompts user to join developers' channels
                    # acbotSettings prompts user to join channels, required by modules
                ),
                None,
            ):
                logger.debug(
