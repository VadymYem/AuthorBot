...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.acbot_caller or client_id == record.acbot_caller)
        ]
    ...
    async def sender(self):
        ...
        self._exc_queue = {
            client_id: [
                ...
                for item in self.tg_buff
                if isinstance(item[0], AcbotException)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_acbot_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_acbot_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
