...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.acbot_me.id
        self._cached_username = (
            self._client.acbot_me.username.lower()
            if self._client.acbot_me.username
            else str(self._client.acbot_me.id)
        )
    ...
    async def _handle_command(
        ...
        if (
            ...
            and message.chat.title.startswith("acbot-")
            and message.chat.title != "acbot-logs"
        ):
            if not watcher:
    ...
    async def future_dispatcher(
        ...
        *args,
        ...
        _acbot_client_id_logging_tag = copy.copy(self.client.tg_id)  # skipcq
        try:
            await func(message)
